...
| 决策 | 设计含义 | 替代方案被淘汰的原因 |
|------|----------|----------------------|
| **单 Repo per account_id** | 同一账号下的 `resources/`、`agent/`、`user/`、`session/` 全部在一棵 root tree 之下;一次 commit 可覆盖任意 scope 的子集 | per-resource repo 会产生 N×账号数量的索引数据,跨 resource 的"事务性快照"需要协调多 repo,复杂度高 |
...
> 💡 **LocalObjectStore 和 S3ObjectStore 直接调用 tokio::fs 和 Arc\<aws_sdk_s3::Client\>, 不复用 LocalFileSystem/S3Client**
>
...
4. **权限边界清晰**：account_id 已经是天然的隔离单位,Git 仓库边界与现有权限模型完全对齐
...
| 层级 | 触发条件 | 节省的开销 | 实现位置 |
|------|----------|------------|----------|
| **Fast Path 1**: Stat 索引复用 oid | 文件 (size, mtime_ns) 与 prev_index 完全一致 | 跳过 `vfs.read` + sha1 hash | **MVP 未实现**(无 commit_index.bin,每次 commit 都 read+hash) |
| **Fast Path 2**: Tree 子树原样保留 | 子树下所有路径都没 upsert/remove | 跳过子树重 hash + 新 tree object 写入 | 已实现(由 `TreeEditor::write` 内部保证) |
...
restore 主流程:**解析目标 commit → 提取该 commit 中 project_dir 子树 → 与当前 HEAD 中同路径子树 diff → 通过 MountableFS.write/rm 回写 → 删除回写后空目录 → 以当前 HEAD 为 parent 生成新 commit → CAS 更新 ref → 把受影响路径返回给调用方**。`dry_run` 模式只计算差异不写,用于预检。
...
>
> - **written_paths / deleted_paths**：`Applied` 响应除了 `written/deleted` 计数外,还返回**全量受影响路径(已加 project_dir 前缀)**;Python 层按 marker / 源文件 / `.relations.json` 分类,精确触发 L0/L1/DETAIL 向量更新,不再依赖广义的 `_trigger_vector_rebuild(paths)`。
>
> - **没有 commit_index 刷新**：对应 §8.1 的 Fast Path 1 未实现,restore 末尾也无须刷新 index。
>
...
- **CAS 冲突**: **当前实现** GitService::commit/restore **内部不做自动重试,直接以 GitError::ConcurrentCommit 上抛给 Python 层,由调用方决定是否 re-read parent 重建 tree 重新提交。git.tuning.ref_cas_max_retry / ref_cas_backoff_ms 配置项已在 GitTuningConfig 中定义但**尚未在代码中读取**,后续接入。
...
| restore 并发 | 同一 account_id 同一时间仅 1 个 restore | 进程内 Mutex,防止 VFS 写竞态 | **未实现** |
...
| 并发 | `ConcurrentCommit` | CAS 失败(由 ref_store conflict 上抛) |
...
| 层级 | 范围 |
|------|------|
| **单元测试 (Rust)** | ObjectStore 各操作的幂等性;RefStore CAS 在并发下的正确性(MVP 用普通并发测,未引入 loom);tree_builder 的 upsert/remove/write;错误映射 |
...
5. **dry_run 不写**: restore dry_run 后再 ls,VFS 状态不变
6. **账号隔离**: A 账号的 commit_oid 在 B 账号下 show 必须返回 not found
...
- **Fast Path 1 (commit_index.bin)** —— 文档 §8.1。无 `commit_index.bin` 持久化,每次 commit 都对候选路径做 `vfs.read` + sha1 hash。
...
- **资源限制实际生效** —— 文档 §12.3。`BlobTooLarge` / `TooManyFiles` / `[git.quota]` 配置块 / 同账号 restore 单飞 Mutex 均未实现(仅错误 variant 已定义)。
- **账号 ID 校验** —— 文档 §12.1。`validate_account_id` 未实现,`GitError::InvalidAccountId` 仅占位。
- **加密绕过(restore 写回)** —— 文档 §12.2。restore 通过 `MountableFS.write` 回写时,如果底层走 `viking_fs.write` 加密路径仍存在双重加密风险;尚未为 restore 路径接入 `raw=True` 或绕过加密的入口。
...
- **VikingFS._trigger_vector_rebuild(account, paths)(早期设计)** 已被更精确的 `_schedule_vector_rebuild(written, deleted)` 替代,**不会**再实现旧 API。
...
6. **向量索引版本化(可选)**: 若后续需要向量索引的快照回滚能力,可引入轻量 manifest 记录 index 版本与对应 commit_oid 的映射,避免全量存储向量数据
