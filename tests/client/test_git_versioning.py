"""End-to-end tests for the OpenViking.git namespace.
...
def _build_binding_client(config_path: Path, fs_root: Path):
    client = ragfs_python.RAGFSBindingClient(config_path=str(config_path))
...
def _build_harness(config_path: Path, fs_root: Path) -> ClientHarness:
    ctx = _make_ctx()
    vfs = VikingFS(agfs=_build_binding_client(config_path, fs_root))
...
def git_harness(workspace) -> ClientHarness:
    ...
    return _build_harness(cfg, fs_root)
...
async def test_write_commit_show_roundtrip(git_harness):
    ...
    commit = git_harness.client.git.commit(
        message="initial",
    ...
    assert git_harness.client.git.show(
        "main",
...
async def test_show_metadata_without_path(git_harness):
    ...
    metadata = git_harness.client.git.show("main")
...
async def test_log_walks_parents(git_harness):
    ...
    for idx, body in enumerate((b"v1", b"v2", b"v3"), start=1):
        ...
        commits.append(
            git_harness.client.git.commit(
                message=f"c{idx}",
                paths=["viking://resources/log.md"],
            )
        )

    history = git_harness.client.git.log(limit=10)
    limited = git_harness.client.git.log(limit=2)
...
async def test_restore_reverts_file_and_advances_head(git_harness):
    ...
    v1 = git_harness.client.git.commit(
        message="v1",
    ...
    v2 = git_harness.client.git.commit(
        message="v2",
        paths=["viking://resources/proj/a.md"],
    )

    restore = git_harness.client.git.restore(
    ...
    assert git_harness.client.git.show("main")["parents"] == [v2["commit_oid"]]
...
async def test_restore_dry_run_does_not_mutate(git_harness):
    ...
    before_log = git_harness.client.git.log()

    dry_run = git_harness.client.git.restore(
        project_dir="viking://resources/proj",
        source_commit=v1["commit_oid"],
        dry_run=True,
    )
    ...
    assert len(git_harness.client.git.log()) == len(before_log)
...
async def test_restore_internal_scope_rejected(git_harness):
    ...
    with pytest.raises(InvalidURIError):
        git_harness.client.git.restore(
...
async def test_disabled_raises_not_supported(git_disabled_harness):
    with pytest.raises(AGFSNotSupportedError):
        git_disabled_harness.client.git.commit(message="disabled")
    with pytest.raises(AGFSNotSupportedError):
        git_disabled_harness.client.git.show("main")
    with pytest.raises(AGFSNotSupportedError):
        git_disabled_harness.client.git.restore(
            project_dir="viking://resources/proj",
            source_commit="main",
        )
    with pytest.raises(AGFSNotSupportedError):
...
async def test_async_api_parity(git_harness):
    ...
    commit = await git_harness.async_client.git.commit(
        message="async initial",
        paths=["viking://resources/async.md"],
    )
    body = await git_harness.async_client.git.show(
        "main",
