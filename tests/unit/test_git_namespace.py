"""Unit tests for AsyncGitNamespace and SyncGitNamespace.
...
from openviking.git_namespace import AsyncGitNamespace, SyncGitNamespace
...
def async_ns(fake_async_client):
    return AsyncGitNamespace(fake_async_client)
...
def test_sync_namespace_delegates_through_async(monkeypatch):
    """SyncGitNamespace.commit() runs the async equivalent via run_async."""
    # Build a fake SyncOpenViking exposing an async_client with a git namespace.
    ...
    sync_parent._async_client.git = inner_async_ns

    sync_ns = SyncGitNamespace(sync_parent)
...
def test_async_client_git_property_is_lazy_and_cached():
    """Accessing .git twice returns the same instance and doesn't construct early."""
    ...
    assert not hasattr(inst, "_git")
    ns1 = inst.git
    assert isinstance(ns1, AsyncGitNamespace)
    ns2 = inst.git
    assert ns1 is ns2


def test_sync_client_git_property_is_lazy_and_cached():
    from openviking.sync_client import SyncOpenViking
    inst = object.__new__(SyncOpenViking)
    assert not hasattr(inst, "_git")
    ns1 = inst.git
    assert isinstance(ns1, SyncGitNamespace)
    ns2 = inst.git
    assert ns1 is ns2
